"""Test del rendering Markdown -> HTML (markdown-it-py)."""
...
def test_nested_list_is_actually_nested():
    html = render("- a\n  - b\n")
    # una lista annidata: due <ul>, con il secondo dentro un <li>
...
def test_inline_code_and_emphasis():
    html = render("Testo con `code`, **bold** e *italic*.")
...
def test_headings_produce_toc_and_anchors():
    html, toc = dp.render_markdown("# Uno\n## Due\n### Tre\n", Path("d.md"))
...
def test_image_is_rendered():
    html = render("![descrizione](pic.png)")
    assert "<img" in html
    assert 'src="pic.png"' in html
    assert 'alt="descrizione"' in html


def test_relative_md_link_rewritten_to_html(tmp_path, monkeypatch):
    monkeypatch.setattr(dp, "ROOT", tmp_path)
    (tmp_path / "altro.md").write_text("x", encoding="utf-8")
    html = render("[vedi](altro.md)", tmp_path / "doc.md")
    assert 'href="altro.html"' in html


def test_external_link_is_untouched():
    html = render("[sito](https://example.com/x)")
